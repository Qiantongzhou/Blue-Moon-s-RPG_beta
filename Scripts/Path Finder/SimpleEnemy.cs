...
/*string temp = pathSequence.Pop();
            if (temp.Substring(0,temp.IndexOf(" ")).Equals("plain"))
            {
                return p.GetDetecter(temp).transform.position;
            }
            else
            {
                return p.GetGate(temp).transform.position;
            }
            */
